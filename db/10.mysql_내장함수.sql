/*
if(수식, 참, 거짓)
ifnull(속성, 값) : 속성값이 null이 아니면 속성값을 이용하고, null이면 값을 이용
case when 조건1 then 값1 when 조건2 then 값2 else 값3
 : 조건1이 참이면 값1을 조건2가  참이면 값2를 아님 값3을 이용
 
instr(기준문자열, 찾을문자열);
 : 기준 문자열에서 찾을 문자열이 몇번째에 있는지 알려주는 함수, 없으면 0 . 1부터 시작
  
format(숫자, 소숫점 자리수);
 : 숫자를 소숫점 자리까지 표현하고, 정수부분은 3개단위로 ',' 를 추가해줌 
...
substring(문자열, 시작위치, 길이);
 : 문자열에서 시작위치부터 길이만큼 찾아서 반환, 길이가 생략되면 시작위치부터 끝가지 반환
