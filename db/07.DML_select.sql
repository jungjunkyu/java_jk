...
like : 와일드 카드와 함께 사용되며, 값이 주어진 와이드가 포함된 문자열과 같은지를 비교할 때 사용
_ : 한글자를 의미하는 와일드카드
% : 0글자 이상을 의미하는 와일드카드
between A and B : 속성의 값이 A이상 B 이하인 조건을 만족할 때 사용
=> 속성명 >= A and 속성명 <= B
...
---ORDER BY---
- 정렬 : order by
- order by 는 where절과 group by절 뒤에 나와야함
...
-- 이순신 교수님과 한석봉 교수님이 강의하는 수강정보를 조회하는 커리
...
WHERE
    subject_code = (SELECT 
        ...
        WHERE
            title = '글쓰기');
-- 학점이 2학점 또는 3학점 이상인 과목의 수강 정보를 조회하는 쿼리를 작성
...
-- 학생정보를 이름순으로 정렬하여 오름차순 정렬하여 조회
select * from student order by name;
-- 학생정보를 이름순으로 정렬하여 내림차순 정렬하여 조회
...
-- 과목별 수강생 수가 4명 이상인 과목코드를 조회하는 쿼리
select subject_code as 과목코드, count(num) as 수강생수 from course group by subject_code having 수강생수 >=4; 
