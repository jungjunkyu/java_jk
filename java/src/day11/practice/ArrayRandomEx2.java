...
public class ArrayRandomEx2 {
	...
	public static void main(String[] args) {
		...
		for(int i =0;i<3;) {
			...
			//checkAr 배열에 random번지에 있는 값이 false이면
			if(!checkArr[random]) {
				//random을 배열에 저장.
				arr[i] = random;
				//i를 증가
		...
		while(count <3) {
			...
			/*위 반복문에ㅓ break를 만나지 않으면 i는 count일 때 종료하기 때문에
	 ...
	 * 매개변수 : 중복을 확인할 배열, 비교할 정수 num, 비교할개수count
	...
	public static boolean contains(int arr[], int num, int count) {
		for(int i=0;i<count;i++) {
			//중복되면 메서드를 종료하면서 중복되었다고 알려줌.	
			if(num ==arr[i]) {
				return true;
			}
			
		}
		//이 위치까지 왔다는건 return true를 못 만났다는 뜻이고, 그 말은  중복되지 않았다는 뜻.
		return false;	
	}
